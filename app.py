...
# ==========================================
# 在这里粘贴你的腾讯文档链接（必须支持任何人编辑）
# ==========================================
TS_URL = "【腾讯文档】hbkpihttps://docs.qq.com/sheet/DWHNYZ1ZUWUd4aWZt?tab=BB08J2"


def load_data():
    try:
        return pd.read_excel(TS_URL, engine="openpyxl")
    except:
        cols = [
            "year", "month", "income_cum", "contract_cum", "renew_rate", "quanyu_cum", "dikong_cum", "shilian_complete",
            "market_share", "huikuan_rate", "shouzhicha_rate",
            "income_score", "contract_score", "renew_score", "new_business_score", "market_score", "shilian_score",
            "delivery_deduct", "business_total",
            "huikuan_score", "zhangqi_total", "shouzhicha_deduct", "quality_total", "plus_total", "deduct_total",
            "key_total", "final_score"
        ]
...
def save_data(new_row):
    try:
        df = pd.read_excel(TS_URL, engine="openpyxl")
    except:
        df = load_data()
...
annual_target = {
    ...
    "new_business_quanyu": 152, "new_business_dikong": 152, "shilian_target": 73, "market_share_target": 0.4,
    "huikuan_target": 0.44, "shouzhicha_target": 0.35
}

st.set_page_config(page_title="鹤壁智联考核系统", layout="wide")
st.title("📊鹤壁智联业务月度考核系统")
st.subheader("✅ 指标测算｜指标分析｜优劣对比")
...
with st.form("form"):
    ...
    submit = st.form_submit_button("✅计算并保存（永久存储）")
...
if submit:
    ...
    con_score = 15 + 5 * min((contract_cum - annual_target["contract_base"]) / (
                annual_target["contract_challenge"] - annual_target["contract_base"]),
                             1) if con_rate >= 1 else 15 * con_rate
    ...
    new_row = pd.DataFrame([[
        year, month, income_cum, contract_cum, renew_rate, quanyu_cum, dikong_cum, shilian_complete, market_share,
        huikuan_rate, shouzhicha_rate,
        inc_score, con_score, renew_score, new_s, mar_score, shi_score, deliv, bus_total, hk_score, z_total, sc_del,
        qual_total, add, sub, item_total, final
...
if len(df_all) > 0:
    show = df_all[
        ["year", "month", "income_score", "contract_score", "renew_score", "new_business_score", "market_score",
         "shilian_score", "business_total", "huikuan_score", "zhangqi_total", "quality_total", "final_score"]].copy()
    show.columns = ["年", "月", "收入", "新签", "续签", "新业务", "市场", "视联", "业务分", "回款", "账期", "效益分",
                    "总分"]
...
if len(df_all) >= 2:
    ...
    for i, j in itertools.combinations(range(len(df_sort)), 2):
        a = df_sort.iloc[i]
        b = df_sort.iloc[j]
        la = f"{int(a.year)}年{int(a.month)}月"
        lb = f"{int(b.year)}年{int(b.month)}月"
        st.markdown(f"### {lb} VS {la}")
        opt, bad = [], []
        tab = []
        for name, col in items:
            ...
            v2 = round(b[col], 2)
            diff = round(v2 - v1, 2)
            tab.append([name, v1, v2, diff])
            if diff > 0: opt.append(f"{name} +{diff}")
            if diff < 0: bad.append(f"{name} {diff}")
        st.dataframe(pd.DataFrame(tab, columns=["指标", "往期", "本期", "增减"]), hide_index=True)
        c1, c2 = st.columns(2)
        c1.success(f"✅ 优化 {len(opt)} 项")
        for x in opt: c1.write(x)
        c2.error(f"❌ 劣化 {len(bad)} 项")
        for x in bad: c2.write(x)
        st.divider()
else:
    st.info("需要至少2个月数据")